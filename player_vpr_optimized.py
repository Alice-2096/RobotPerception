...
from vpr_new import generate_histogram, train_vocab, best_match, compare_histograms_filter_by_threshold
...
class KeyboardPlayerPyGame(Player):
    ...
    def find_loops(self):
        """
        loop closure in the live image list using VPR: 
            for every 5 index i in the live image list, find the best match image 10 indices after i. If such an image exists, then we have found a loop closure. 

            in the shortcut list, set the shortcut[i] = index of the best match image
            Otherwise, leave it as -1 (no shortcut)
        """
        print('finding loops ...')
        hist_list = [item['hist'] for item in self.live_image_list]
        kmeans = self.kmeans
        for i in range(0, len(hist_list), 3):
            delta = i + 10
            if delta >= len(hist_list):
                break
            best_match_index = compare_histograms_filter_by_threshold(
                hist_list[i], hist_list[delta:], delta, threshold=0.6)
            if best_match_index > 0:
                self.shortcut[i] = best_match_index
                print(f'Found loop closure: {i} -> {self.shortcut[i]}')
                # show two images in another window
                cv2.imshow(f'Loop Closure #{i}',
                           self.live_image_list[i]['image'])
                cv2.imshow(f'Loop Closure #{self.shortcut[i]}',
                           self.live_image_list[self.shortcut[i]]['image'])
                cv2.waitKey(1)
            else:
                self.shortcut[i] = -1
